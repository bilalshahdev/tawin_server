...
export const checkout = asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.status(201).json(new ApiResponse(req.t('order.placed'), order));
});
...
export const listOrders = asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.status(200).json(new ApiResponse(req.t('order.list_retrieved'), data, meta));
});
...
export const getOrder = asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.status(200).json(new ApiResponse(req.t('order.fetched'), order));
});

export const changeStatus = asyncHandler(async (req: Request, res: Response) => {
    const order = await orderService.updateStatus(req.params.id as string, req.body.status);
    return res.status(200).json(new ApiResponse(req.t('order.updated'), order));
});
...
export const removeOrder = asyncHandler(async (req: Request, res: Response) => {
    ...
    return res.status(200).json(new ApiResponse(req.t('order.deleted')));
});
